...
from lib.dataset import spatial_transforms

class AUC_CL(data.Dataset):
    def __init__(self, split = 'train', imSize=(224,224),aug=False):
        ...
        self.crop_scales=[f/100 for f in range(80,100)]
        
        self.transformImg_S = spatial_transforms.Compose([
                #spatial_transforms.MultiScaleCornerCrop(self.crop_scales, 256, crop_positions=['c']),
                spatial_transforms.MultiScaleRandomCrop(self.crop_scales, 256),
    ...
    def __getitem__(self, index):
        ...
        self.transformImg_S.randomize_parameters()
        img_seq_1=[self.transformImg_S(img) for img in img_seq]
        img_seq_1=[self.transformImg(np.uint8(img)) for img in img_seq_1]
        img_seq_1=torch.stack(img_seq_1,dim=0)
        img_seq_1=img_seq_1.permute(1,0,2,3)#(channel, t, h, w)
        
        img_seq_1=img_seq_1.squeeze()
        
        self.transformImg_S.randomize_parameters()
        img_seq_2=[self.transformImg_S(img) for img in img_seq]
        img_seq_2=[self.transformImg(np.uint8(img)) for img in img_seq_2]
        img_seq_2=torch.stack(img_seq_2,dim=0)
        img_seq_2=img_seq_2.permute(1,0,2,3)#(channel, t, h, w)
        
        img_seq_2=img_seq_2.squeeze()
        '''
        #load optical flow
        flow_seq=[]
        ...
        return img_seq_1,img_seq_2,img_label #img_seq, flow_seq, pose_xy,img_label
    ...
    def _load_samples(self,is_train=False):
        ...
        for row in reader:
            ...
            ''' 
            #filter the frames
            if is_train and img_idx in test_labels[row['Label']]:
               continue
            '''
            
            if is_train and int(row['Label'])>2:
               continue
