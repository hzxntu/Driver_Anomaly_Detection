...
def get_video(video_path, frame_indices):
    ...
    """
    image_reader = pil_loader #get_default_image_loader()
    video = []
...
def make_dataset(root_path, subset, view, sample_duration, type=None):
    ...
    if subset == 'train' and type == 'normal':
        ...
        for train_folder in train_folder_list:
            ...
            for normal_video in normal_video_list:
                ...
                #for i in range(0, n_frames):
                    ...
                    for idx,k in enumerate(sample_['frame_indices']):
                        if not os.path.exists('%s/img_%d.png'%(video_path,k)):
                           sample_['frame_indices'][idx]=k-1
        ...
        for train_folder in train_folder_list:
            ...
            for anormal_video in anormal_video_list:
                ...
                #for i in range(0, n_frames):
                    ...
                    for idx,k in enumerate(sample_['frame_indices']):
                        if not os.path.exists('%s/img_%d.png'%(video_path,k)):
                           print('No: ','%s/img_%d.png'%(video_path,k))
                           sample_['frame_indices'][idx]=k-1
                    if len(sample_['frame_indices']) < sample_duration:
                        for j in range(sample_duration-len(sample_['frame_indices'])):
                            sample_['frame_indices'].append(sample_['frame_indices'][-1])
                    ...
                    dataset.append(sample_)
    elif subset == 'train' and type == 'normal_anormal':
        # load normal training data
        train_folder_list = list(filter(lambda string: string.find('Tester') != -1, list(listdir(root_path))))
        ...
        for train_folder in train_folder_list:
            ...
            for normal_video in normal_video_list:
                ...
                for i in range(0, n_frames, int(sample_duration)):
                #for i in range(0, n_frames):
                    sample_ = sample.copy()
                    sample_['frame_indices'] = list(range(i, min(n_frames, i + sample_duration)))
                    for idx,k in enumerate(sample_['frame_indices']):
        ...
        for train_folder in train_folder_list:
            ...
            for v_idx, anormal_video in enumerate(anormal_video_list):
                ...
                for i in range(0, n_frames, int(sample_duration/16)):
                #for i in range(0, n_frames):
                    sample_ = sample.copy()
                    sample_['frame_indices'] = list(range(i, min(n_frames, i + sample_duration)))
                    for idx,k in enumerate(sample_['frame_indices']):
    ...
    print('loaded DAD dataset %s : '%subset,len(dataset))
    return dataset


class DAD_CL(data.Dataset):
    """
    generate normal training/ anormal training/ validation dataset according to requirement
    """
    def __init__(self,
                 root_path='./dataset/DAD/DAD/',
                 subset='train',
                 view='front_depth',
                 sample_duration=16,
                 type=None,
    ...
    def __getitem__(self, index):
        if self.subset == 'train':
            ...
            #print(frame_indices)
            clip_org = self.loader(video_path, frame_indices)
            
            self.spatial_transform.randomize_parameters()
            clip_1 = [self.spatial_transform(img) for img in clip_org]
            #clip = torch.stack(clip, 0).permute(1, 0, 2, 3)                 #data with shape (channels, timesteps, height, width)
            #clip_1 = torch.stack(clip_1, 0).permute(3, 0, 1, 2)                 #data with shape (channels, timesteps, height, width)
            clip_1 = torch.stack(clip_1, 0).unsqueeze(0)                 #data with shape (channels, timesteps, height, width)
            self.spatial_transform.randomize_parameters()
            clip_2 = [self.spatial_transform(img) for img in clip_org]
            #clip = torch.stack(clip, 0).permute(1, 0, 2, 3)                 #data with shape (channels, timesteps, height, width)
            #clip_2 = torch.stack(clip_2, 0).permute(3, 0, 1, 2)                 #data with shape (channels, timesteps, height, width)
            clip_2 = torch.stack(clip_2, 0).unsqueeze(0)                 #data with shape (channels, timesteps, height, width)
            
            ground_truth=torch.FloatTensor(np.array([ground_truth]))
            return clip_1, clip_2, ground_truth
            ...
            clip = torch.stack(clip, 0).unsqueeze(0)                 #data with shape (channels, timesteps, height, width)
            ground_truth=torch.FloatTensor(np.array([ground_truth]))
